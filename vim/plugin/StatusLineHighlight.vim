...
" modified Fri Jul  6 16:42:54 CDT 2012
" to change bg to white, which is reversed to become fg for better visibility
" made other changes to readonly and special
function! s:DefaultHighlightings()
    highlight def StatusLineModified           term=bold,reverse cterm=bold,reverse ctermbg=white ctermfg=DarkRed  gui=bold,reverse guifg=DarkRed guibg=white
    highlight def StatusLineModifiedNC         term=reverse      cterm=reverse      ctermfg=DarkRed  gui=reverse      guifg=DarkRed
    highlight def StatusLinePreview            term=bold,reverse cterm=bold,reverse ctermfg=Blue     gui=bold,reverse guifg=Blue ctermbg=white guibg=white
    highlight def StatusLinePreviewNC          term=reverse      cterm=reverse      ctermfg=Blue     gui=reverse      guifg=Blue
    highlight def StatusLineReadonly           term=bold,reverse cterm=reverse ctermfg=Grey     gui=reverse guifg=DarkGrey
    highlight def StatusLineReadonlyNC         term=reverse      cterm=bold,reverse      ctermfg=Grey     gui=bold,reverse      guifg=DarkGrey
    highlight def StatusLineSpecial            term=bold,reverse cterm=bold,reverse ctermfg=DarkBlue gui=bold,reverse guifg=DarkBlue ctermbg=white guibg=white
    highlight def StatusLineSpecialNC          term=reverse      cterm=reverse      ctermfg=DarkBlue gui=reverse      guifg=DarkBlue guibg=Grey
